...
public class TrelloAppService {
    ...
    /**
     * Instansiates a Trello app based on data stored in string.xml and saved preferences.
     * <strong>Only use this function to retrieve the Trello App</strong>
     * @param context
     * @return a Trello app
     */
     ...
     */
    public static TrelloImproved getTrelloAPIInterface(TrelloApp trelloApp) {
        Log.d("debug", "Creating new trello API interface (APIKey: " + trelloApp.getApplicationID()
                + ", Token: " + trelloApp.getAuthenticationToken());
        return new TrelloImplImproved(trelloApp.getApplicationID(), trelloApp.getAuthenticationToken());
    }
